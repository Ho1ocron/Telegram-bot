...
def subj_choice(self):
    global ssubj
    global text1
    text1.append(self.text)
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    btn = [types.KeyboardButton for types.KeyboardButton in generator.sub.keys()]
    markup.add(*btn)
    if len(text1) == 1 or len(text1) == 0:
        if not self.text in commands.keys():
...
def send_doc(self):
    ...
    if not self.text in subj_ch[ssubj].keys():
        ...
        try:
            ...
            bot.send_message(self.chat.id, "Вот ваш документ. /start", reply_markup=markup)
            bot.register_next_step_handler(self, start)
            
        except:
            print("He удалось отправить документ.")
            bot.send_message(self.chat.id, "He удалось отправить документ.", reply_markup=markup)
            bot.register_next_step_handler(self, start)


commands = {word: "Выбери класс." for word in generator.sub.keys()}
